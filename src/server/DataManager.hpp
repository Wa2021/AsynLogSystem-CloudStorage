...
namespace storage
{
    ...
    private:
        ...
        bool need_persist_;//持久化开关，在构造函数中先设置为false，
        // 否则每 Insert 一条从磁盘读出的数据，程序又会立刻调用 Storage() 把它写回磁盘。
        // 这会导致在启动阶段产生大量无效的磁盘 I/O，严重降低启动速度。
    ...
    public:
        ...
        bool Delete(const std::string &url)
        {
            ...
            {
                pthread_rwlock_unlock(&rwlock_);
                mylog::GetLogger("asynclogger")->Warn("Delete fail, url not found");
                return false;
            }
            table_.erase(it);
            if (need_persist_ == true && Storage() == false)
