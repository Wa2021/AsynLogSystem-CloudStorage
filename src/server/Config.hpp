...
namespace storage
{
    ...
    private:
        ...
        std::string auth_salt_;        // 密码盐值
        std::string auth_hash_;        // 密码哈希值
    ...
    public:
        ...
        bool ReadConfig()
        {
            ...
            // 读取认证信息
            if (root.isMember("auth_salt") && root.isMember("auth_hash")) {
                auth_salt_ = root["auth_salt"].asString();
                auth_hash_ = root["auth_hash"].asString();
            } else {
                // 默认使用 admin / CloudStorageSalt2026
                auth_salt_ = "CloudStorageSalt2026";
                auth_hash_ = "177ef1e3769364729e37cf38f9810a31f64d92d3362356760e81dc4d24aae124";
            }
            return true;
        }
        int GetServerPort()
